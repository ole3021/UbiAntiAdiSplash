...
{
    static USplash * spl = NULL;
    if(uMsg == WM_CREATE)
    {
        spl = (USplash*)((LPCREATESTRUCT)lParam)->lpCreateParams;
    }
    if(spl)
...
LRESULT CALLBACK USplash::WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
void USplash:: OnPaint(HWND hwnd)
{
    if (!m_hBitmap)
        return;
...
void USplash::Init()
{
    ...
    //  =======================================================================
    m_hwnd = NULL;
    m_lpszClassName = TEXT("Ubisoft Splash");
    m_colTrans = 0;
...
USplash::USplash(LPCTSTR lpszFileName, COLORREF colTrans, LPCTSTR lpszLocation)
{
    Init();

    SetBitmap(lpszFileName);
    SetTransparentColor(colTrans);
	SetLaunchApplication(lpszLocation);
}
...
HWND USplash::RegAndCreateWindow()
{
    //  =======================================================================
...
DWORD USplash::SetBitmap(LPCTSTR lpszFileName)
{
    //  =======================================================================
    //  load the bitmap
    //  =======================================================================
...
DWORD USplash::SetBitmap(HBITMAP hBitmap)
{
    int nRetValue;
    BITMAP  csBitmapSize;
...
DWORD USplash::SetLaunchApplication(LPCTSTR location) {
	m_appLocation = location;

	return 1;
}

void USplash::FreeResources()
{
    if (m_hBitmap)
...
bool USplash::SetTransparentColor(COLORREF col)
{
    m_colTrans = col;
...
bool USplash::LaunchApplication() {
	ShellExecute(NULL, TEXT("open"), m_appLocation,
